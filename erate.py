...
def dashboard():
    ...
    try:
        query = db.query(Erate)
        state = request.args.get('state', '').strip().upper()
        if state:
            query = query.filter(Erate.state == state)

        total_filtered = query.count()
        data = query.order_by(Erate.app_number).limit(11).all()
...
def import_interactive():
    if not os.path.exists(CSV_FILE):
        return "<h2>CSV File Not Found</h2><p>Upload 470schema.csv to src/</p>", 404
    ...
    if request.method == 'POST':
        ...
        if action == 'import' and confirm == 'ok':
            return _import_one_record(progress)
        elif action == 'import_all':
            return _import_all_records(progress)
...
# === IMPORT ONE RECORD ===
def _import_one_record(progress):
    db = next(SessionLocal())
    try:
        with open(CSV_FILE, 'r', encoding='utf-8-sig') as f:
            reader = csv.DictReader(f)
        ...
        if not app_number:
            ...
            progress['index'] += 1
            session['import_progress'] = progress
            return render_template('erate_import.html', row=row, progress=progress, success=False, error="Missing Application Number")

        if db.get(Erate, app_number):
            progress['error'] += 1
            progress['index'] += 1
            session['import_progress'] = progress
            return render_template('erate_import.html', row=row, progress=progress, success=False, error="Already exists")
        ...
        progress['error'] += 1
        session['import_progress'] = progress
        return render_template('erate_import.html', row=row, progress=progress, success=False, error=str(e))
    finally:
        db.close()

    progress['index'] += 1
    session['import_progress'] = progress
    return render_template('erate_import.html', row=row, progress=progress, success=True, error=None)

# === BULK IMPORT ===
def _import_all_records(progress):
    db = next(SessionLocal())
    try:
        with open(CSV_FILE, 'r', encoding='utf-8-sig') as f:
            reader = csv.DictReader(f)
